...
def main():
    ...
    model = build_module(
        ...
        in_channels = vae.get_out_channels(),
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                # save_video_grid(x.float()*0.5+0.5, "vis_data2.mp4", nrow=None, fps=8)
                # save_sample(x[0], fps=8 , save_path="vis_data3.mp4")
                y = batch.pop("text")
                ...
                with torch.no_grad():
                    ...
                    h = vae.codebook.idxBl_to_f_hat(x, same_shape=True, last_one=True)
                    # print ("h: ", h.shape)
                    #h = self.post_vq_conv(shift_dim(h, -1, 1))
                    h = h.to(dtype)

                    x = vae.post_vq_conv(h)
                    # Prepare text inputs
...
def save_video_grid(video, fname, nrow=None, fps=20):
