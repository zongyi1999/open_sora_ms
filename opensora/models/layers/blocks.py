...
class CodecEmbedder(nn.Module):
    ...
    def token_drop(self, codec, force_drop_ids=None):
        """
        Drops labels to enable classifier-free guidance.
        """
        if force_drop_ids is None:
            drop_ids = torch.rand(codec.shape[0]).cuda() < self.uncond_prob
        else:
            drop_ids = force_drop_ids == 1
        codec = torch.where(drop_ids[:, None, None, None], self.y_embedding, codec)
        return codec

    def forward(self, codec,): #train, force_drop_ids=None):
        # if train:
        #     assert caption.shape[2:] == self.y_embedding.shape
        # use_dropout = self.uncond_prob > 0
        # if (train and use_dropout) or (force_drop_ids is not None):
        #     caption = self.token_drop(caption, force_drop_ids)
        caption = self.y_proj(codec)
        return caption
