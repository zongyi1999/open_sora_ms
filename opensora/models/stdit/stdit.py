...
class STDiT(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None, x_mask=None,codec1=None):
        """
        ...
        if codec1 is not None:
            codec1 = rearrange(codec1, "B C T H W -> B (H W) T C")
            codec1 = self.c_embedder(codec1)
            mask_codec = torch.ones((B, T,),dtype=int).cuda()
            mask = torch.cat((mask_codec, mask),dim=1)
            y = torch.cat((codec1, y),dim=2)
